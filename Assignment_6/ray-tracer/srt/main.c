...
scene_t scene;
int numThreads;
float pixelData[width][height][3];

/* Multithread implementation */

void* multiThreadIMPL(void* start){
  ...
  /* for every pixel 
     Loop unrolled so work is split between threads */
  for( int px=startVal; px<width; px += numThreads /* threads */ ){
	  ...
	  */
	  /*Multithreading causes printing order mistakes so needs fixing
	   */
	  pixelData[px][py][0] = scaled_color[0];
	  pixelData[px][py][1] = scaled_color[1];
	  pixelData[px][py][2] = scaled_color[2];
...
main( int argc, char **argv )
{
    ...
    int* num = (int*) malloc(sizeof(int) * numThreads);
    
    int i;
    for(i=0; i<numThreads; i++){
      num[i] = i;
      int check = pthread_create(&threads[i], NULL, multiThreadIMPL, &num[i]);
      if(check){
    ...
    for(i=0; i<width; i++){
      for(j=0; j<height; j++){
	printf( "%.0f %.0f %.0f\n",
		pixelData[i][j][0], pixelData[i][j][1], pixelData[i][j][2] );
      }
      printf("\n");
    }
